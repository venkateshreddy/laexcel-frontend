...
import PreAdmissionsForm from './PreAdmissionForm';
...
class PreAdmission extends Component {
  ...
  uploadValues = () => {
    ...
    if (
      columns.indexOf('Email') < 0 ||
      columns.indexOf('ContactNumber') < 0 ||
      columns.indexOf('Program') < 0 ||
      columns.indexOf('StudentName') < 0
    ) {
      this.props.dispatch(
        handleSnackBar({
          snackBarOpen: true,
          snackBarMsg:
            'Mandatory columns missing. Please see the note above the table.'
        })
      );
      ...
      this.props.dispatch(
        handleSnackBar({
          snackBarOpen: true,
          snackBarMsg: 'Source type can not be empty'
        })
      );
      ...
      this.props.dispatch(
        handleSnackBar({
          snackBarOpen: true,
          snackBarMsg: 'Agency code can not be empty'
        })
      );
      ...
      this.props.dispatch(
        handleSnackBar({
          snackBarOpen: true,
          snackBarMsg: `You've not uploaded any rows`
        })
      );
    } else {
      ...
      this.props
        ...
        .catch(err => {
          this.props.dispatch(
            handleSnackBar({
              snackBarOpen: true,
              snackBarMsg: `Upload Failed. Please try again.`
            })
          );
        });
    }
  ...
  onDrop = acceptedFiles => {
    const opts = {
      errors: {
        badfile() {
          alert(
            'This file does not appear to be a valid Excel file. If we made a mistake, please send this file to <a href="mailto:saravanakvp1983@gmail.com?subject=I+broke+your+stuff">saravanakvp1983@gmail.com</a> so we can take a look.',
            () => {}
          );
        },
        pending() {
        ...
        failed(e) {
          alert(
            'We unfortunately dropped the ball here.  We noticed some issues with the grid recently, so please test the file using the <a href="/js-xlsx/">raw parser</a>.  If there are issues with the file processor, please send this file to <a href="mailto:venkatesh_kalikiri@mcbitsstech.com?subject=I+broke+your+stuff">venkatesh_kalikiri@mcbitsstech.com</a> so we can make things right.',
            () => {}
          );
        }
      },
      on: {
        sheet: (json, sheetnames) => {
          ...
          headerArr.map(column => {
            if (
              ['StudentName', 'Program', 'ContactNumber', 'Email'].indexOf(
                column
              ) < 0
            ) {
    ...
    function fixdata(data) {
      ...
      o += String.fromCharCode.apply(
        null,
        new Uint8Array(data.slice(o.length))
      );
      return o;
    }
    ...
    reader.onload = function(e) {
      ...
      if (e.target.result.length > 1e6)
        opts.errors.large(e.target.result.length, e => {
          if (e) doit();
        });
      else {
        doit();
  ...
  render() {
    ...
    return (
      <Tabs
        ...
        mountOnEnter
      >
        <Tab eventKey={'FORM'} title="Manual Entry">
          <PreAdmissionsForm />
        </Tab>
          ...
          <Row>
            <Col lg={12} md={12} sm={12} xs={12}>
              <div className="browse-wrap padding">
                ...
                <Col lg={6} md={6} sm={6} xs={6}>
                  <div class="form-group has-feedback">
                    <label for="agencyCode" class="control-label">
                      Agency Code *
                    </label>
                    <input
                ...
                <Col lg={6} md={6} sm={6} xs={6}>
                  <span style={{ fontSize: '10px' }}>
                    Note*: The below given default fields are mandatory in excel
                    sheet.
                  </span>
                </Col>
                ...
                <Col>
                  {this.state.rows.length > 0 && (
                    <div>
                      <button
                        type="button"
                        className="btn btn-custom btn-rounded btn-bordered waves-effect w-xxs waves-light ml-2"
                      ...
                      <button
                        type="button"
                        className="btn btn-primary btn-rounded btn-bordered waves-effect w-xxs waves-light ml-2"
        ...
        <Tab eventKey={'Data'} title="View Data">
          <PreAdmissionsTable />
        </Tab>
      </Tabs>
    );
